    ...
    def save_entries(self):
        if self.username:
            ...
            try:
                ...
                print(f"Error writing entries file: {str(e)}")
...
class DiaryApp:
    def __init__(self, root):
        ...
        self.login_button = tk.Button(self.menu_frame, text="Login", command=self.login, padx=20, pady=10)
        self.login_button.pack()

        self.create_user_button = tk.Button(self.menu_frame, text="Create User", command=self.create_user, padx=20, pady=10)
        ...
        self.exit_button = tk.Button(self.menu_frame, text="Exit", command=self.save_and_exit, width=16, height=4, state=tk.DISABLED)
        self.exit_button.pack()

        self.color_button = tk.Button(self.menu_frame, text="Change Colors", command=self.change_colors, width=15, height=2)
        self.color_button.pack()

        self.random_colors = False

        def create_button(self, text, command, state=tk.NORMAL, width=15, height=2):
            button = tk.Button(self.menu_frame, text=text, command=command, state=state, width=width, height=height)
            button.pack()
            return button
    ...
    def change_colors(self):
        if not self.random_colors:
            ...
            self.root.configure(bg=self.generate_random_color())
            self.menu_frame.configure(bg=self.generate_random_color())
            for button in [
                self.login_button, self.create_user_button, self.entry_button,
                self.view_button, self.lock_button, self.unlock_button,
                self.update_button, self.find_button, self.delete_button,
                self.exit_button, self.color_button
            ]:
                button.configure(bg=self.generate_random_color())
        else:
            self.random_colors = False
